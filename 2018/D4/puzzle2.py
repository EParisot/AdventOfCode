...
for turn in turns:
    for key, val in turn.items():
        if key not in guards_sleep:
            guards_sleep[key] = [0 if i == "1" else 1 for i in val]
        else:
            for i, elem in enumerate(guards_sleep[key]):
                guards_sleep[key][i] += 0 if val[i] == "1" else 1

print("\n", guards_sleep)
sleepy_guard = max(guards_sleep, key=lambda key: max(guards_sleep[key]))
print("\n", sleepy_guard)
sleepy_minute = guards_sleep[sleepy_guard].index(max(guards_sleep[sleepy_guard]))
print("\n", sleepy_minute)

print("\n", int(sleepy_guard[1:]) * sleepy_minute)
